...
class PatientMoodHistoryScreen extends StatefulWidget {
  ...
  @override
  State<PatientMoodHistoryScreen> createState() => _PatientMoodHistoryScreenState();
}
...
class _PatientMoodHistoryScreenState extends State<PatientMoodHistoryScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final entries = await _specialistService.getPatientMoodEntries(widget.patientId);
      if (!mounted) return;
      setState(() {
        _entries = entries;
        _loading = false;
      ...
      body: _loading
          ...
          : _error != null
              ...
              : _entries.isEmpty
                  ? Center(
                      child: Text(
                        'No mood entries found for ${widget.patientName}.',
